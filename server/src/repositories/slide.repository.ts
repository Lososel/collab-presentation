...
export const getSlidesByPresentationId = async (
...
export const updateSlideContent = async (
  ...
  const result = await db.query(
    'UPDATE slides SET content = $1 WHERE id = $2 RETURNING *',
    [content, slideId]
  );
  return result.rows[0];
};

export const updateSlidePosition = async (
  slideId: string,
  position: number
): Promise<Slide> => {
  const result = await db.query(
    'UPDATE slides SET position = $1 WHERE id = $2 RETURNING *',
    [position, slideId]
  );
  return result.rows[0];
};
